...
export default function DashboardOverview({ userId }: OverviewProps) {
  ...
  const remainingBudget = useMemo(() => {
    return budgets.reduce((sum, budget) => sum + Number(budget.limit_amount || 0), 0) - totalExpenses;
    ...
    return budgets
      .map((budget) => {
        ...
        const spent = expenses
          .filter((expense) => expense.category_id === budget.category_id && matchesRange(expense.date))
          .reduce((sum, expense) => sum + Number(expense.amount || 0), 0);
        const used = budget.limit_amount > 0 ? Math.min((spent / budget.limit_amount) * 100, 100) : 0;
  ...
  }, [budgets, categories, expenses, dateRange]);
          ...
          <CardContent className="space-y-5">
            <div className="grid gap-5 lg:grid-cols-[220px_1fr]">
              <div className="flex items-center justify-center">
                <div
                  ...
                  style={{
                    background: `conic-gradient(#2f855a 0% 34%, #6fbf73 34% 57%, #9dd7a7 57% 76%, #d7ead7 76% 100%)`,
                  }}
                >
                  <div className="flex h-32 w-32 items-center justify-center rounded-full bg-background text-center shadow-inner">
                    <div>
                      ...
                      <div className="text-2xl font-bold text-foreground">
                        {budgets.length ? Math.round((totalExpenses / (budgets.reduce((sum, item) => sum + Number(item.limit_amount || 0), 0) || 1)) * 100) : 0}%
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-wrap gap-3">
              <div className="rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm text-foreground">
                <span className="text-muted-foreground">Spending:</span> ${totalExpenses.toFixed(2)}
              </div>
              <div className="rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm text-foreground">
                <span className="text-muted-foreground">Budget limit:</span> $
                {budgets.reduce((sum, item) => sum + Number(item.limit_amount || 0), 0).toFixed(2)}
              </div>
            </div>
          </CardContent>
        </Card>
